...
function HomeComponent(props) {
  ...
  const recentLoadingDiv = () => {
    ...
    for (let i = 0; i < 8; i++) {
      items.push(
        <div key={i} className="h-16 w-full rounded-md bg-light-bg text-white justify-start items-center inline-flex">
  ...
  for (let i = 0; i < 8; i++) {
    items.push(
      <div key={i} className="h-6/12 w-5/12 rounded-md justify-start items-center flex-shrink-0 text-sm pr-4 leading-tight">
  ...
  return items
    ...
    return (
      <div className="z-0 w-full absolute top-20 bg-dark-bg h-max bg-fixed font-SpotifyCircular-Light">
      <div className="mx-4">
